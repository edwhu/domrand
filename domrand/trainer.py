...
from domrand.utils.models import XYZModel, BinnedModel, PoseModel, forward
from domrand.utils.general import notify
from domrand.utils.data import load_eval_data,load_all_eval_data, parse_record, brighten_image, bin_label
        ...
        while True:
            ...
            try:
                ...
                if FLAGS.plot_preds:
                    ...
                    eval_plots = np.array([make_pred_plot(img, pred, label, mode=FLAGS.output) for k, (img, pred, label) in enumerate(zipped)])
...
def model_eval():
    ...
    file_eval_imgs, file_eval_labels = load_all_eval_data(FLAGS.real_data_path, FLAGS.real_data_shape)
    eval_img_ph = tf.placeholder(file_eval_imgs.dtype, file_eval_imgs.shape)
    eval_label_ph = tf.placeholder(file_eval_labels.dtype, file_eval_labels.shape)
    pred_plot_ph = tf.placeholder(file_eval_imgs.dtype, PLOT_SHAPE)
    eval_dataset = tf.data.Dataset.from_tensor_slices((eval_img_ph, eval_label_ph))
    ...
    with tf.Session(config=config) as sess:
        ...
        if FLAGS.plot_preds:
            ...
            #idxs = np.linspace(0,50,50).astype(int)
            #zipped = [zipped[idx] for idx in idxs]
            eval_plots = np.array([make_pred_plot(img, pred, label, mode=FLAGS.output, name=f"{k}.png") for k, (img, pred, label) in enumerate(zipped)])
            plot_summary = sess.run([viz_eval_summary], {pred_plot_ph:eval_plots})[0]
            import ipdb; ipdb.set_trace()

        # record down the x, y, zrot average error and 1 std
        xy_diff = np.abs(eval_preds[:, :2] - eval_labels[:, :2])
        xy_error = np.mean(xy_diff)
        xy_std = np.std(xy_diff)
        print("XY error:", xy_error, xy_std)

        x_error, y_error = np.mean(xy_diff, axis=0)
        x_std, y_std = np.std(xy_diff, axis=0)
        print("X error:", x_error, x_std)
        print("Y error:", y_error, y_std)

        z_diff = np.abs(eval_preds[:, 2] - eval_labels[:, 2])
        z_error = np.mean(z_diff)
        z_std = np.std(z_diff)
        print("z error:", z_error, z_std)

        print("-" * 80)
        print("Averaged eval")
        ep_len = 10
        num_eps = len(eval_preds) // ep_len
        # for each episode of 10 images, get the average estimate
        average_eval_preds = []
        for i in range(num_eps):
            start = i * ep_len
            end = (i+1) * ep_len
            average_eval_preds.append(np.mean(eval_preds[start:end], axis=0))
        average_eval_preds = np.array(average_eval_preds)
        print(average_eval_preds.shape)
        assert average_eval_preds.shape == (num_eps, 3)


        # compute average error of mean vs pose estimates
        xyz_diff = []
        for i, avg_estimate in enumerate(average_eval_preds):
            ep_idx = i * ep_len
            xyz_diff.append(np.abs(eval_preds[ep_idx: ep_idx+ep_len]- avg_estimate))

        xyz_diff = np.concatenate(xyz_diff)
        assert xyz_diff.shape == (len(eval_preds), 3)
        # get only 3 camera viewpoints
        xyz_diff_cams = []
        for i in range(num_eps):
            camera_idx = [0,5,9]
            xyz_diff_cams.append(xyz_diff[i * ep_len: (i + 1) * ep_len][camera_idx])
        import ipdb; ipdb.set_trace()

        xyz_diff = np.concatenate(xyz_diff_cams)
        xy_diff = xyz_diff[:, :2]
        xy_error = np.mean(xy_diff)
        xy_std = np.std(xy_diff)
        print("XY error:", xy_error, xy_std)
